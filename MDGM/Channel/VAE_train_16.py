...
parser.add_argument('--exp',type = str ,default = 'Channel_16',help = 'dataset')
...
dir = os.getcwd()
directory = f'/Channel/experiments/experiments_16/latent16/beta_{args.beta_vae}'
...
def loss_function(recon_x, x, mu, logvar):
    rencon_loss = F.mse_loss(recon_x.view(-1,256), x.view(-1,256), size_average=False)
    mu=mu.reshape(-1,16)
...
for epoch in range(1,args.n_epochs+1):
    ...
    if (epoch) % args.sample_interval == 0:
        torch.save(decoder.state_dict(), output_dir + f'/decoder_16_VAE_{args.beta_vae}_epoch{epoch}.pth')
        torch.save(encoder.state_dict(), output_dir + f'/encoder_16_VAE_{args.beta_vae}_epoch{epoch}.pth')
