...
parser.add_argument('--exp',type = str ,default = 'Channel_16_32',help = 'dataset')
...
dir = os.getcwd()
directory = f'/Channel/experiments/experiments_16_32/latent64/beta_{args.beta_vae}'
...
def loss_function(recon_x, x, mu1, logvar1):
    rencon_loss = F.mse_loss(recon_x.view(-1,1024), x.view(-1,1024), size_average=False)
    mu1=mu1.reshape(-1,64)
    logvar1=logvar1.reshape(-1,64)
    KLD = torch.sum(-0.5 * torch.sum(1 + logvar1 - mu1.pow(2) - logvar1.exp(), dim = 1), dim = 0)
...
for epoch in range(1,args.n_epochs+1):
    ...
    decoder.train()
    train_loss = 0  
    for batch_idx, (x32_data,x16_data) in enumerate(train_loader):
        x32_data = x32_data.to(device)
        x16_data = x16_data.to(device)
        optimizer.zero_grad() 
        z, mu, logvar = encoder(x32_data)
        _,z16,_ = X_l_encoder(encoder_model,x16_data)
        z16 = upsample(z16,16)
        z_h = torch.cat((z16,z),1)
        recon_batch = decoder(z_h)
        loss,rec_loss, kl_loss= loss_function(recon_batch, x32_data,  mu, logvar)
        loss.backward()
        train_loss += loss.item()
        optimizer.step()
        print('Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f} recon_loss:{:.6f} kl_loss:{:.6f}'.format(
                epoch, batch_idx * len(x32_data), len(train_loader.dataset),
                100. * batch_idx / len(train_loader),
                loss.item() / len(x32_data),rec_loss.item() / len(x32_data),kl_loss.item() / len(x32_data)))
    batches_done = epoch * len(train_loader) + batch_idx
    if (epoch) % args.sample_interval == 0:
        torch.save(decoder.state_dict(), output_dir + f'/decoder_16_32_VAE_1_{args.beta_vae}_epoch{epoch}.pth')
        torch.save(encoder.state_dict(), output_dir + f'/encoder_16_32_VAE_1_{args.beta_vae}_epoch{epoch}.pth')
