...
class mean_std_plot(object):
    ...
    def compute_mean_std(self,pos,truth,save_dir):
        ...
        if self.exp =='inversion_16_32' or self.exp =='inversion_16_64':
            zc = pos[:16,:]
            zf = pos[16:,:]
            for i in range(num):
        ...
        if self.exp =='inversion_16_32_64':
            zc = pos[:80,:]
            zf = pos[80:,:]
            for i in range(num):
                pos_samples[i,:] = self.prediction.permeability(self.exp,model,zc[:,i],zf=zf[:,i]).cpu().numpy().reshape(self.scale*self.scale,)
    ...
    def plot(self,truth,mean,std,sample1,sample2,sample3,save_dir):
        ...
        plt.savefig(save_dir+f'/channel_statistics_{self.exp}.pdf',bbox_inches = 'tight', dpi=1000,pad_inches=0.0)
        plt.close()
