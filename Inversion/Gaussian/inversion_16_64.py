...
if __name__ == "__main__":
    ...
    parser.add_argument('--steps',type = int ,default = 5000,help = 'steps')
    parser.add_argument('--burn_in',type = int ,default = 3000,help = 'burn_in')
    ...
    parser.add_argument('--exp',type = str ,default = 'inversion_16_64',help = 'scale_size')
    args = parser.parse_args()
    device = torch.device("cuda:2" if torch.cuda.is_available() else "cpu")
    latent_c = latent_c(device)
    mean_c = np.loadtxt(os.getcwd()+f'/Gaussian/prediction/inversion_16/mean.dat')
    ...
    sigma = np.loadtxt(os.getcwd()+f'/Gaussian/test_data/sigma_{args.noise}.dat')
    true_permeability = np.loadtxt(os.getcwd()+f'/Gaussian/test_data/true_permeability_{args.noise}.dat')
    true_pressure = np.loadtxt(os.getcwd()+f'/Gaussian/test_data/true_pressure_{args.noise}.dat')
    obs_position = np.loadtxt(os.getcwd()+f'/Gaussian/test_data/obs_position_{args.noise}.dat')
    ...
    if args.proposal == 'pCN':
        beta = 0.08
    output_dir = os.getcwd()+ f'/Gaussian/plot_inversion_state/{args.exp}'
    ...
    save_dir = os.getcwd()+f'/Gaussian/prediction/{args.exp}'
    ...
    # plot error bar #
    filename = '/gaussian_16_64_errorbar.pdf'
